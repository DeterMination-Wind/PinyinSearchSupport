...
修复图纸搜索在 v159 下恒为“未找到”的问题。

### 图纸搜索修复

- **蓝图卡片匹配改走数据源**：图纸搜索不再依赖 UI 文本提取，而是直接按 `Schematic.name()` 做拼音、首字母与模糊匹配，避免 v159 蓝图卡片结构变化导致全部结果丢失。
- **标签筛选保持一致**：在当前已选标签范围内继续做拼音过滤，结果顺序与原版图纸面板保持一致。
- **回车使用首个结果**：过滤后同步更新 `firstSchematic`，保持原版回车直接选择第一项的行为。
- **TextField 反射状态修复**：临时清空搜索框再恢复时，游标和选区会按文本长度修正并恢复，避免输入状态异常。
